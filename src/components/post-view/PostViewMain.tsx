    ...
    return (
        ...
        <div className="px-6 md:px-[12%] py-10 flex flex-col gap-4">
            <div className="flex justify-between">
                <Link to={`/profile/${post?.username}`} className='text-blue-600 font-semibold no-underline hover:underline'>{post?.username}</Link>
                <div className='flex flex-col text-[12px] items-end text-gray-600'>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {post?.tag?.split(",")?.map((tag, i) => (
                            <Link key={i} to={`/search/${tag.toLowerCase()}`} className="py-[2px] rounded-md px-3 text-sm bg-blue-50 text-blue-600 hover:bg-blue-100 no-underline">#{tag}</Link>
                        ))}
                    </div>
            <Comment slug={slug} />
        </div >
